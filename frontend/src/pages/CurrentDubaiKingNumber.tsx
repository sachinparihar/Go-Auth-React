// CurrentDubaiKingsNumber.tsx
import React, { useState, useEffect, useRef } from 'react';
import axios from 'axios';
import waitImage from './img/wait.gif'; // Import the wait image

const CurrentDubaiKingsNumber = () => {
  const [currentNumber, setCurrentNumber] = useState<number | null>(() => JSON.parse(localStorage.getItem('dubaiKingsCurrentNumber') || 'null'));
  ...
  useEffect(() => {
    ...
    if (currentNumber === null) {
      axios.get('http://localhost:8000/api/getdubaikings') // Replace with Dubai Kings API endpoint
        .then(response => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('dubaiKingsCurrentNumber', JSON.stringify(currentNumber));
...
export default CurrentDubaiKingsNumber;
export {};